...
$(document).ready(function () {
  ...
  $('button').on('click', function () {
    ...
    if (text.length > 0) {
      ...
      // set teh border color to this style
...
function openAPIConnect(processText) {
  return new Promise(function (resolve, reject) {
    ...
    let openAIKey = "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";  // ADD YOU OPENAPI KEY KEY HERE
    ...
    let serviceModel = "text-davinci-003";           
    // service endpoint                       
    let serviceEndpoint = 'https://api.openai.com/v1/completions';
    ...
    serviceRequest.onreadystatechange = function () {
      ...
      if (serviceRequest.readyState === 4) { // 4 means request is completed
        ...
        // For simplicity, we only use the first response element. 
          ...
          let responseMessage = JSON.parse(serviceRequest.responseText);
          let responseText = responseMessage.choices[0].text.trim();
